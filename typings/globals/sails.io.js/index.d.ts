...
// Source: https://raw.githubusercontent.com/arvitaly/sails.io.js-typed/master/global.d.ts
declare namespace SailsIOJS {
    ...
    export interface ISocket {
        ...
        request<T>(options: IRequestOptions, cb?: IRequestCallback<T>): void;
        on(event: string, cb: (...args) => any): ISocket;
        on(event: "connect", cb: () => any): ISocket;
        on(event: "disconnect", cb: () => any): ISocket;
        on(event: "reconnecting", cb: (numAttempts: number) => any): ISocket;
        on(event: "reconnect", cb: (transport: string, numAttempts: number) => any): ISocket;
